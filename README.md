...
- We can grab the `form` and listen for events on it:
...
```html
  <form id="form">
    <input type="text" placeholder="New Show..." /> <br />
    <button type="submit">Add to Watch List</button>
  </form>
```

- This form ⬆️ will try to send a `POST` request to `/watchlist`
...
- [Cross-Site Scripting Attacks](https://en.wikipedia.org/wiki/Cross-site_scripting)
